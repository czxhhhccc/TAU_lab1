...
def Step_no_PID():
    ...
    W_ForwardGain = W1*W2*W3*matlab.tf([1.714],[1])
    print('W_ForwardGain',W_ForwardGain)
    W_cc = matlab.feedback(W_ForwardGain, 1, sign=-1)
    print('W_cc',W_cc)
    timeLine = []
    for i in range(0, 5000):
        timeLine.append(i / 200)
        pass
    [y, x] = matlab.step(W_cc, timeLine)
    plt.plot(x,y)
    plt.grid()
    x_ticks = np.linspace(0,25,78)
    plt.xticks(x_ticks)
    plt.show()
    # print('pole',W_cc.pole())
    # matlab.nyquist(W_ForwardGain)
    # plt.show()
...
def test1_PID():
    ...
    for ii in kp:
        ...
        # plt.show()
        pass
# test1_PID()   # из этого теста получается Kp примерно 0,4
def test2_PID():
    Ti = np.linspace(15,30,150)
    for ii in Ti:
        plt.clf()
        timeLine = []
        for i in range(0, 1000):
...
def test3_PID():
    ...
    for ii in Td:
        ...
        for i in range(0, 1000):
            timeLine.append(i / 10)
            pass
        W1,W1_open = PID(0.25,25,ii)
        [y, x] = matlab.step(W1, timeLine)
        plt.plot(x,y)
        plt.plot([11,11],plt.ylim(), 'r--')
        plt.plot(plt.xlim(), [0.95, 0.95], 'r--')
        plt.plot(plt.xlim(), [1.05, 1.05], 'r--')
        plt.title('iter:{}'.format(ii))  # title 可以直观展现出  迭代进行到哪一步了
        plt.grid()
        plt.pause(0.001)
        # plt.show()
        pass
# test3_PID()   # 0.25 25 1
W_PID,W_PID_Open = PID(0.25,25,1.2)   #1,20,2/0.25,25,1.2
def Step(W):
...
def Direct_Assess(W):
    ...
    :return:
    '''
    timeLine = np.linspace(0,200,6000)
    [y,x] = matlab.step(W,timeLine)
    y = list(y)
    x = list(x)
...
def solve_w_c(n,W, omega_limits = [0.001,10000]):
    ...
    if len(mag_dB2)>0:   # если вторая группа не пуста
...
def plot2(w_c, phase_0,W, omega_limits = [0.001,10000]):
    ...
    plt.plot([w_c, w_c], plt.ylim(), 'r--')
    plt.title("Фаза при частоте среза = {0:.3g} rad".format(phase_0))
...
# log_feature(W_PI)
# matlab.nyquist(W_PID_Open,plot=True)
# plt.show()
def Аmplitude_Frequency(W):
    x, f2_, M_ = M(W)
    ...
    if len(x_slice_index)>0:
        x_slice = x[min(x_slice_index)]
        print('Время регулирования: {:.3f}'.format(1*np.pi/x_slice) + 'с')
    else:
        print('Частота среза равна 0 rad')
    print(x_slice_index)
# Аmplitude_Frequency(W_PI)   # передаточная характеристика замкнутой системы

def Ite(W):
    timeLine = np.linspace(0, 100, 2000)
    [y, x] = matlab.step(W, timeLine)
    yy = []
    for i in y:
        yy.append(1-i)
    ...
    for i in y:
        yyy.append(fabs(1-i))
    I2 = integrate.trapezoid(yyy, x)
    print('Линейная интегральная оценка, которая равна площади, заключенной между прямой X(∞) и кривой X(t) равна ',I1)
    print('Модульная интегральная оценка', I2)
Ite(W_PI)